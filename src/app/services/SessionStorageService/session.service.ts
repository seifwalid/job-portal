...
export class SessionStorageService {
  ...
  /**
   * Retrieves the user's UID from session storage.
   * @returns The user's role, or null if session storage is not available or the role does not exist.
   */
  getRole(): string | null {
    return this.get('role')?.toString() ?? null;
  }

  async setUserToSessionStorage(): Promise<void> {
    const u = await this.userService.getUser(this.getUid()!);
    u && this.save("role", u.role);
  }
